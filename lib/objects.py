...
class Weather(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.bot_name = kwargs.get('bot_name', 'booneweather')
        self.get_wu_weather()
    ...
    def get_yahoo_weather(self):
        ...
        self.r.set(self.bot_name + ':tomorrow:high', tomorrow.getAttribute('high'))
        self.r.set(self.bot_name + ':tomorrow:low', tomorrow.getAttribute('low'))
        self.r.set(self.bot_name + ':today:high', today.getAttribute('high'))
        self.r.set(self.bot_name + ':today:low', today.getAttribute('low'))
    ...
    def get_wu_weather(self):
        ...
        self.r.set(self.bot_name + ':current:temp', conditions[0].split(': ')[1].split('&')[0].strip())
        self.r.set(self.bot_name + ':current:cond', conditions[3].split(': ')[1].split('&')[0].lower().strip())
        self.r.set(self.bot_name + ':current:wind_direction', conditions[4].split(': ')[1].split('&')[0].strip())
        self.r.set(self.bot_name + ':current:wind_speed', conditions[5].split(': ')[1].split('&')[0].strip())
